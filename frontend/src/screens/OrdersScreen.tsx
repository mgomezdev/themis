...
import type { Order, Job } from '../data/types';
    ...
    >
      <div className="row between" style={{ alignItems: 'flex-start', gap: 8 }}>
        <div className="col" style={{ minWidth: 0, flex: 1 }}>
          ...
          <div style={{ fontWeight: 500, marginTop: 2, fontSize: 13.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {order.customer}
          </div>
          <div className="tiny muted" style={{ marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {order.title}
          </div>
        </div>
...
function OrderDetail({ order }: { order: Order }) {
  ...
  const relatedJobs: Job[] = JOBS.filter(j => j.parts.some(pr => pr.orderId === order.id));
    ...
    <div className="col gap-4" style={{ minWidth: 0 }}>
      <div className="card" style={{ padding: 20 }}>
        <div className="row between" style={{ alignItems: 'flex-start' }}>
          ...
          <div className="row gap-2">
            <StatusPill status={order.status} />
            <button className="btn sm">{Icons.copy} Clone</button>
            <button className="btn icon sm">{Icons.more}</button>
          </div>
        </div>

        <div className="row gap-6" style={{ marginTop: 18, flexWrap: 'wrap' }}>
          <Kv k="Placed"    v={<span className="num small">{order.placed}</span>} />
          <Kv k="Due"       v={<span className="num small">{order.due}</span>} />
          <Kv k="Parts"     v={<span className="num small">{printedParts}<span className="muted"> / {totalParts}</span></span>} />
          <Kv k="Remaining" v={<span className="num small">{fmtTime(totalTime)}</span>} />
          <Kv k="Jobs"      v={<span className="num small">{relatedJobs.length}</span>} />
        </div>

        {order.notes && (
          <div style={{ marginTop: 16, padding: 12, background: 'var(--bg-1)', borderRadius: 8, borderLeft: '2px solid var(--accent)', color: 'var(--text-2)', fontSize: 13 }}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border-1)' }}>
          <SectionHeader title="Parts breakdown" sub={`${order.parts.length} unique parts`} actions={<button className="btn sm">{Icons.plus} Add part</button>} />
        </div>
        <table className="tbl">
          <thead>
            <tr>
          ...
          <tbody>
            {order.parts.map(p => {
              ...
              return (
                <tr key={p.id}>
                  <td style={{ width: 40 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 4, background: p.thumbColor, border: '1px solid var(--border-1)', backgroundImage: 'linear-gradient(135deg, rgba(255,255,255,0.08), transparent 50%)' }} />
                  </td>
                  <td>
                  ...
                  <td>
                    ...
                    <div style={{ width: 90, marginTop: 4 }}><Progress value={Math.round((p.printed / p.qty) * 100)} /></div>
                  </td>
                  <td>
                    {p.printed === p.qty
                      ? <StatusPill status="complete" />
                      : p.printed > 0
                        ? <StatusPill status="partial" label={`${p.printed} done · ${remaining} to go`} />
                        : <StatusPill status="queued" />
                    }
                  </td>
                  <td className="num small" style={{ textAlign: 'right' }}>{fmtTime(p.est)}</td>
                  <td className="num small" style={{ textAlign: 'right', color: remaining > 0 ? 'var(--text-1)' : 'var(--text-3)' }}>
                    {remaining > 0 ? fmtTime(remainTime) : '—'}
                  </td>
                </tr>
              );
            })}
      ...
      <div className="card" style={{ padding: 20 }}>
        <SectionHeader title="Jobs filling this order" sub="Plates that include parts from this order" />
        {relatedJobs.length === 0 && <Empty title="No jobs yet" icon={Icons.queue} />}
              ...
              <div key={j.id} className="card" style={{ padding: 12, background: 'var(--bg-1)' }}>
                <div className="row between">
                  ...
                  <div className="row gap-3">
                    ...
                    <span className="num tiny">{fmtTime(j.estTime)}</span>
                  </div>
                </div>
              </div>
            );
...
// =========================================================================
// OrderAccordion — expanded inline body
// =========================================================================

function OrderExpandedBody({ order, relatedJobs }: { order: Order; relatedJobs: Job[] }) {
  return (
    <div style={{ borderTop: '1px solid var(--border-1)', background: 'var(--bg-1)' }}>
      ...
      <div className="row between" style={{ padding: '14px 18px', alignItems: 'flex-start' }}>
        <div className="col" style={{ flex: 1, marginRight: 16 }}>
          {order.notes ? (
            <div style={{ padding: 12, background: 'var(--bg-2)', borderRadius: 8, borderLeft: '2px solid var(--accent)', color: 'var(--text-2)', fontSize: 13, maxWidth: 700 }}>
              <span className="tag-key" style={{ display: 'block', marginBottom: 4 }}>Notes</span>
              {order.notes}
            </div>
          ) : (
        ...
        <div className="row gap-2" style={{ flexShrink: 0 }}>
          <button className="btn sm" onClick={e => e.stopPropagation()}>{Icons.copy} Clone</button>
          <button className="btn sm" onClick={e => e.stopPropagation()}>{Icons.plus} Add part</button>
          <button className="btn icon sm" onClick={e => e.stopPropagation()}>{Icons.more}</button>
        </div>
      </div>
            ...
            {order.parts.map(p => {
              ...
              return (
                <tr key={p.id} style={{ cursor: 'default' }}>
                  ...
                  <td>
                    <div className="num small"><span style={{ color: 'var(--text-1)' }}>{p.printed}</span> / {p.qty}</div>
                    <div style={{ width: 100, marginTop: 4 }}><Progress value={Math.round((p.printed / p.qty) * 100)} /></div>
                  </td>
                  <td>
                    {p.printed === p.qty
                      ? <StatusPill status="complete" />
                  ...
                  <td className="num small" style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>{fmtTime(p.est)}</td>
                  <td className="num small" style={{ textAlign: 'right', whiteSpace: 'nowrap', paddingRight: 18, color: remaining > 0 ? 'var(--text-1)' : 'var(--text-3)' }}>
                    {remaining > 0 ? fmtTime(remainTime) : '—'}
                  </td>
                </tr>
              );
      ...
      {relatedJobs.length > 0 && (
        <div style={{ padding: '14px 18px 18px', borderTop: '1px solid var(--border-1)', background: 'var(--bg-0)' }}>
              ...
              return (
                <div key={j.id} className="row between" style={{ padding: '10px 12px', background: 'var(--bg-2)', borderRadius: 8, border: '1px solid var(--border-1)' }}>
                  <div className="row gap-3" style={{ alignItems: 'center', minWidth: 0 }}>
                    ...
                    <div style={{ fontWeight: 500, fontSize: 13, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{j.plateName}</div>
                    <StatusPill status={j.status} />
                  </div>
                  <div className="row gap-4" style={{ flexShrink: 0 }}>
                    ...
                    <span className="num tiny" style={{ whiteSpace: 'nowrap' }}>{fmtTime(j.estTime)}</span>
                  </div>
                </div>
              );
            })}
...
function OrderAccordion({ order, expanded, onToggle }: { order: Order; expanded: boolean; onToggle: () => void }) {
  const totalParts = order.parts.reduce((a, b) => a + b.qty, 0);
  const printedParts = order.parts.reduce((a, b) => a + b.printed, 0);
  const pct = totalParts > 0 ? Math.round((printedParts / totalParts) * 100) : 0;
    ...
    <div className="card" style={{ padding: 0, overflow: 'hidden', borderColor: expanded ? 'var(--border-3)' : 'var(--border-1)' }}>
      ...
      >
        <div aria-hidden="true" className="row gap-4" style={{ alignItems: 'center' }}>
          {/* chevron */}
          <div style={{ width: 20, color: 'var(--text-3)', transform: expanded ? 'rotate(90deg)' : 'none', transition: 'transform 160ms ease', display: 'inline-flex' }}>
            {Icons.chevR}
          </div>
          ...
          <div className="col" style={{ width: 130, flexShrink: 0 }}>
            ...
            <span aria-hidden="true" className="tiny" style={{
              padding: '1px 6px',
              borderRadius: 4,
              background: order.type === 'internal' ? 'rgba(99,102,241,0.12)' : 'rgba(56,189,248,0.12)',
              color: order.type === 'internal' ? '#a5b4fc' : 'var(--info)',
          ...
          <div className="col" style={{ width: 80, flexShrink: 0, alignItems: 'flex-end' }}>
            ...
            <span className="num small" style={{ marginTop: 2, whiteSpace: 'nowrap', color: remainingTime === 0 ? 'var(--text-3)' : 'var(--text-1)' }}>
              {remainingTime > 0 ? fmtTime(remainingTime) : '—'}
            </span>
          </div>
...
// =========================================================================
// OrdersScreen — exported main component
// =========================================================================

export function OrdersScreen() {
  const [expanded, setExpanded] = useState<string | null>(null);
  const [filter, setFilter] = useState<'open' | 'all' | 'customer' | 'internal'>('open');
      ...
      <div className="row gap-2">
        {([['open', 'Open'], ['all', 'All'], ['customer', 'Customer'], ['internal', 'Internal']] as const).map(([k, l]) => (
          <button
            ...
            style={filter === k ? undefined : { background: 'transparent', borderColor: 'var(--border-1)' }}
          >{l}</button>
        ))}
        <div style={{ flex: 1 }} />
