...
| File | Exports |
|---|---|
| `queue.ts` | Types `ApiJob`, `ApiJobDetails`, `ApiJobPrinterConfig` (`tool_index: number \| null`; `filament_map?: {model_filament:number, tool_index:number}[] \| null`), `ApiSliceFailure`, `ApiPlate`, `ModelFilament` (`index:number, color:string, type:string`). `PrinterConfigInput` includes `filament_map?`. `useQueue()` (list + `/ws` `job_update`/`queue_update` merge). `useFilePlates(ids)` (cached plate metadata). `createJob`, `cancelJob`, `unblockJob`, `updateJobConfigs`, `getJobDetails`, `getSliceFailures`, `reorderQueue`, `uploadFile`, `getPrinterProfiles`, `getModelFilaments(fileId)` (→ `GET /files/{id}/model-filaments`), `checkOverrides`, `plateThumbnailUrl`. |
