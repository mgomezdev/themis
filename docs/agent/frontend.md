...
| `JobDetailScreen` | `/jobs/:id` — full read view: thumbnail, file/plate stats, per-printer slicing config (incl. slice errors, and a low-stock warning block from that config's `low_stock_warning` — computed per `job_printer_configs` row, independent of whether the job's been assigned), assigned printer, editable filament-cost field (`setJobCost`, any job status); actions: edit / unblock / cancel. |
| `OrdersScreen` | `useOrders` accordion; per-order derived progress bar, parts table (Part/Material/Qty/Est), amount-paid/payment-status column + expanded paid/filament-cost line, linked jobs (clickable → `/jobs/:id`), Hold/Edit/Delete. |
| `NewOrderScreen` | Create **and** edit (`/orders/:id/edit` via `useParams`). Parts table with Spoolman-aware filament picker, amount-paid + payment-status inputs; `createOrder`/`updateOrder`. |
...
| `ProjectBuilderScreen` | `/projects/new` and `/projects/:id/edit` (same component, `useParams` branches create vs edit) — assemble a project's items (`FilamentRequirementPicker`, `PrinterEligibilityPicker`, `ProcessPresetPicker`), links, non-printed parts, and amount-paid/payment-status fields before generating. |
| `ProjectDetailScreen` | `/projects/:id` — read view: payment-status badge + amount paid in the header, aggregated filament-cost total, linked jobs with live status (`STATUS_META`), parts checklist (`allocated` toggle → `updateProjectPart`), links, Generate. |
...
| File | Exports |
|---|---|
| `queue.ts` | Types `ApiJob` (incl. `low_stock_warning: LowStockWarning \| null`, `filament_cost: number \| null`), `ApiJobDetails`, `ApiJobPrinterConfig` (`tool_index: number \| null`; `filament_map?: {model_filament:number, tool_index:number}[] \| null`; `low_stock_warning: LowStockWarning \| null` — per-config, independent of `ApiJob`'s own copy), `ApiSliceFailure`, `ApiPlate`, `ModelFilament` (`index:number, color:string, type:string`), `LowStockWarning` (`{spool_id, spool_label, remaining_g, needed_g, message}`). `PrinterConfigInput` includes `filament_map?`. `useQueue()` (list + `/ws` `job_update`/`queue_update` merge). `useFilePlates(ids)` (cached plate metadata). `createJob`, `cancelJob`, `unblockJob`, `updateJobConfigs`, `getJobDetails`, `setJobCost(id, cost)` (→ `PATCH /jobs/{id}/cost`), `getSliceFailures`, `reorderQueue`, `uploadFile`, `getPrinterProfiles`, `getModelFilaments(fileId)` (→ `GET /files/{id}/model-filaments`), `checkOverrides`, `plateThumbnailUrl`. |
| `orders.ts` | `ApiOrder`(status:`StatusKey`, progress 0..1, `amount_paid: number \| null`, `payment_status: PaymentStatus`, `filament_cost_total: number \| null`), `ApiOrderDetail`(+jobs), `ApiOrderPart`(+filament_id/color). `PaymentStatus = 'unpaid'\|'partial'\|'paid'`. `useOrders()` (refetch on `/ws` job/queue events). `getOrders/getOrder/createOrder/updateOrder/deleteOrder`. |
| `projects.ts` | Types `Project` (incl. `customer/order_type/on_hold/due_date/amount_paid/payment_status/filament_cost_total` — see `data-model.md`), `PaymentStatus`, `ProjectItem`, `ProjectLink`, `ProjectPart`, `ProjectJob`, `GenerateOut`. `useProjects()`. Full CRUD for the project itself + its three child collections (items/links/parts), `generateProject(id, ...)`, `getProjectJobs`. Mirrors `backend/app/api/routes/projects.py` closely — see `backend.md`'s Routes table for the endpoint list. |
