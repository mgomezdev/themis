...
// =========================================================================
// Filament library — filterable catalog of physical spools + print profiles
// =========================================================================

function FilamentsScreen({ tweaks }) {
  const [type, setType] = useState("all");          // all | PLA | PETG | PA-CF | ABS | ASA | PC | TPU
  const [maker, setMaker] = useState("all");        // all | <manufacturer>
  const [query, setQuery] = useState("");
  const [favOnly, setFavOnly] = useState(false);
  const [selected, setSelected] = useState(null);   // filament id
  const [, bumpVersion] = useState(0);              // re-render trigger after mutating filament.favorite
...
function FilSummary({ label, value, sub, pillTone }) {
  return (
...
function FilChip({ active, children, onClick }) {
  return (
...
function Star({ filled }) {
  return (
    <svg width="13" height="13" viewBox="0 0 24 24"
         ...
         stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" style={{ display: "inline-block" }}>
      <path d="M12 2l3.1 6.3 7 1-5 4.9 1.2 6.8L12 17.8 5.7 21l1.2-6.8-5-4.9 7-1z" />
    </svg>
  );
}

function FavoriteToggle({ filled, onClick, size = 16 }) {
  return (
    <button
      ...
      onMouseEnter={(e) => { e.currentTarget.style.color = filled ? "var(--warn)" : "var(--warn)"; }}
      onMouseLeave={(e) => { e.currentTarget.style.color = filled ? "var(--warn)" : "var(--text-4)"; }}>
...
function FilamentCard({ filament: f, selected, onClick, onToggleFavorite }) {
  const printers = useMemo(() => {
    const ids = new Set((f.profiles || []).map(p => p.printerId));
      ...
      <div className="row gap-3" style={{ alignItems: "center" }}>
        ...
        <div className="col" style={{ flex: "1 1 0", minWidth: 0, gap: 3 }}>
          <div className="row gap-2" style={{ alignItems: "baseline", minWidth: 0 }}>
            ...
            {f.favorite && (
              <FavoriteToggle filled onClick={onToggleFavorite} size={14} />
            )}
          </div>
        ...
        {/* profiles -> printer eligibility */}
        <div style={{ width: 130, flexShrink: 0 }}>
          <div className="row gap-2" style={{ flexWrap: "wrap" }}>
            {window.PRINTERS.map(p => (
              <span key={p.id} className={`elig ${printers.includes(p.id) ? "on" : "off"}`} title={p.name}>
                {p.badge}
              </span>
            ))}
          </div>
...
function FilamentDetailPanel({ filament: f, onClose, onToggleFavorite }) {
  return (
    <div className="card" style={{ position: "sticky", top: 0, padding: 18, height: "fit-content" }}>
      ...
      <div className="col gap-2" style={{ marginTop: 8 }}>
        {f.profiles.map((p, i) => {
          const printer = window.getPrinter(p.printerId);
          return (
            <div key={i} style={{
                padding: "10px 12px",
                background: "var(--bg-1)",
        ...
        {/* + New profile — placeholder tile at the bottom of the list */}
        <button
          ...
          {React.cloneElement(window.Icons.plus, { size: 14 })}
      ...
      <button className="btn ghost sm" style={{color:"var(--err)", justifyContent: "center"}}>
        {window.Icons.trash} Archive filament
      </button>
    </div>
  );
}
...
function SpoolSwatch({ color, size = 40 }) {
  // A small spool icon — concentric rings with the filament color in the middle.
  const s = size;
...
Object.assign(window, { FilamentsScreen });
