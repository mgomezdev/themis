...
// =========================================================================
// Themis — icons + shared UI components
// =========================================================================

// ----------- Icons (lucide-style, hand-coded inline SVG) -----------
const Icon = ({ d, paths, children, size = 18, stroke = 1.6, ...rest }) => (
...
function Progress({ value = 0, tone, large }) {
  const cls = tone === "warn" ? "warn" : tone === "err" ? "err" : "";
  return (
    <div className={`progress ${cls} ${large ? "lg" : ""}`}>
      <div className="bar" style={{ "--p": `${Math.max(0, Math.min(100, value))}%` }} />
    </div>
  );
}

function Swatch({ color, large }) {
  return <span className={`swatch ${large ? "lg" : ""}`} style={{ "--c": color }} />;
}
...
function VideoTile({ live = true, status, time }) {
  return (
    <div className={`video ${live ? "live" : ""}`}>
      <div className="feed-scene" />
      <div className="feed-noise" />
      {time != null && <div className="feed-time mono">{window.fmtClock(time)}</div>}
      {status && (
...
function PrinterBadge({ printerId, size = "sm" }) {
  const p = window.getPrinter(printerId);
  if (!p) return null;
  return (
    <span className="elig on" title={p.name}>
      {p.badge}
    </span>
  );
}

function EligibilityChips({ ids }) {
  const all = window.PRINTERS;
  return (
...
// Topbar
function Topbar({ title, crumbs = [], actions, search = true }) {
  return (
...
// Sidebar
function Sidebar({ route, setRoute, nav, queueCount, ordersOpen, filamentsCount }) {
  const items = [
    { id: "queue",     label: "Job queue",        icon: Icons.queue,    count: queueCount },
    { id: "fleet",     label: "Fleet",            icon: Icons.fleet },
    { id: "orders",    label: "Orders",           icon: Icons.orders,   count: ordersOpen },
    { id: "files",     label: "Files",            icon: Icons.files },
    { id: "filaments", label: "Filaments",        icon: Icons.spool },
    { id: "printers",  label: "Printers",         icon: Icons.printer },
  ];
  return (
...
Object.assign(window, {
  Icon, Icons, StatusPill, Progress, Swatch, MaterialChip,
  VideoTile, Card, PrinterBadge, EligibilityChips,
  SectionHeader, Empty, Topbar, Sidebar,
});
